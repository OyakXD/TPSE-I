...
#define GPIO1_DEBOUCINGTIME      0x154
...
volatile bool flag_gpio3 = true;
volatile bool flag_gpio4 = true;
...
void gpioIsr(void)
{
    ...
    if (status & (1 << 16)) {
        ...
        // limpa flag de interrupção só do bit 16
    ...
    if (status & (1 << 17)) {
        ...
        // limpa flag de interrupção só do bit 17
        HWREG(SOC_GPIO_1_REGS + GPIO_IRQSTATUS_0) = (1 << 17);
    }

    if (status & (1 << 12)) {
        flag_gpio4 = false;

        // limpa flag de interrupção só do bit 12
...
void DMTimerIsr()
{
    ...
    // limpa flag de interrupção do timer
