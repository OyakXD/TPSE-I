...
void timerInit()
{
    ...
    // =========================================================================
    //                      /* CONFIGS FOR TIMER2 */ (Cronômetro)
    // =========================================================================
    /* Configures the interface to posted mode for better write latency
       bit 2 (posted) enable posted mode*/
    HWREG(SOC_DMTIMER_2_REGS + DMTIMER_TSICR) |= DMTIMER_TSICR_POSTED;

    // Ensures the timer is stopped before configuration
    while(HWREG(SOC_DMTIMER_2_REGS + DMTIMER_TWPS) & DMTIMER_TPWS_W_PEND_TCLR); // waits for past synchronizations

    HWREG(SOC_DMTIMER_2_REGS+DMTIMER_TCLR) &= ~DMTIMER_TCLR_ST; // disable timer

    /* ENABLE AUTO-RELOAD */
    HWREG(SOC_DMTIMER_2_REGS + DMTIMER_TCLR) |= (1 << 1);

    // Resets the counter and load register
    while(HWREG(SOC_DMTIMER_2_REGS+DMTIMER_TWPS) & DMTIMER_TPWS_W_PEND_TCRR); // waits for past synchronizations

    /* reset counter and load register */
    HWREG(SOC_DMTIMER_2_REGS+DMTIMER_TCRR) = 0x00000000;
    HWREG(SOC_DMTIMER_2_REGS+DMTIMER_TLDR) = 0x00000000;
}
